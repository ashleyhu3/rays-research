"""Render the US tech sector review to HTML.

Numbers come from data/agg_{DATE}.json (never retyped by hand); prose and catalyst
attribution come from narrative/{DATE}.json. Keeping the two apart is what stops the
write-up from drifting away from the tape — every figure in a table is computed, and
every figure quoted in prose is one a reader can check against the table above it.
...
def render_s2(agg: dict, nar: dict) -> str:
    rows = []
...
def render_s4(agg: dict, nar: dict) -> str:
    ...
    for entry in nar["s4"]:
        ...
        # Korea anchors quote in KRW; a dollar-volume figure there would be misleading.
        meta = f"收 {close}" if entry.get("tag") == "韩国锚" else f"收 {close} · 成交 {num(vol)}B$"
...
def render(agg: dict, nar: dict) -> str:
    u = agg["universe"]
    session = date.fromisoformat(agg["session"])
    subtitle = (f'{agg["session"]} ET · {u["locked_n"]} 票 + {len(agg["indices"])} 指数 · '
    ...
    body = "".join([
        ...
        section(2, "板块速览（涨方 ↑ / 跌方 ↓ 双向）", render_s2(agg, nar)),
        section(3, f'个股表 × {u["sectors_total"]} 子赛道', render_s3(agg, nar)),
        section(4, "高亮归因（聚焦 Top ~30 · 极端票 + 反向代表）", render_s4(agg, nar)),
...
def main() -> int:
    ...
    out = REPORT_DIR / f"{args.date}_us_tech_daily.html"
    out.write_text(render(agg, nar), encoding="utf-8")
