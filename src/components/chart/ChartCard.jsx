...
import { ExpandButton, ChartModal } from './ChartExpand';
...
export default function ChartCard({
  ...
  // centralized in src/config/chart{Text,Sources,Insights}.js keyed by chartId.
  // Any prop passed explicitly still wins, so views can override with computed
  // (live-data-dependent) values.
  ...
  // Body (table or chart) rendered at a caller-chosen height, so the same markup
  // serves both the in-grid card (fixed `height`) and the expanded overlay (tall).
  const renderBody = (h) => (
    showTable && transposed ? (
      <div className="ch-table-wrap" style={{ maxHeight: typeof h === 'number' ? Math.max(h, 320) : h }}>
      ...
      <div className="ch-table-wrap" style={{ maxHeight: h }}>
      ...
      <div style={{ position: 'relative', height: h }}>{content}</div>
    )
  );
