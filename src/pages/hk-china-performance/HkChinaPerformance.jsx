...
export default function HkChinaPerformance() {
  ...
  return (
    <>
      {controls}
      <div className="usp-section-label">Indices</div>
      <div className="cgrid">
        <ChartCard
      ...
      {!error && indexRatioCharts.length > 0 &&
      ...
      {!error && sectionCharts.map(section => (
        <div key={section.title}>
          <div className="usp-section-label">{section.title}</div>
          {ratioGrid(section.charts)}
        </div>
      ))}
    </>
  );
