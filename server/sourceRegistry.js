...
const SOURCE_REGISTRY = {
  ...
  openrouterRanks: {
    ...
    ourCadenceMs: 6 * 3600e3, sourceCadence: 'Daily (intraday refresh)', criticalLagThresholdMs: 40 * 3600e3,
    upstreamLagMs: 24 * 3600e3, upstreamLagText: '~1 complete UTC day',
    upstreamLagNote: 'OpenRouter logs generation traffic at its gateway and publishes model-level daily token totals. The dashboard requests data only through yesterday so every bar represents a complete UTC day.',
    reliabilityGrade: 'A-', reliabilityNote: 'Authoritative for real LLM usage share, but only covers traffic routed through OpenRouter; the latest displayed point is the prior complete UTC day.',
    ragScope: 'Real model/provider usage share by daily model-level token throughput.',
    fallback: 'Stored payload (≤36h) served on cold start; latest complete day used.',
    endpointUrl: 'https://openrouter.ai/api/v1/datasets/rankings-daily',
  },
  litellm: {
  ...
  nand: {
    name: 'NAND Flash Spot Prices', provider: 'TrendForce',
    ourCadenceMs: 6 * 3600e3, sourceCadence: 'Per trading session', criticalLagThresholdMs: 18 * 3600e3,
    upstreamLagMs: 8 * 3600e3, upstreamLagText: '~same session (hours)',
    upstreamLagNote: 'TrendForce publishes public NAND flash, wafer, and memory-card spot tables by trading session. We snapshot the posted session price once per UTC day and forward-fill gaps between observed sessions.',
    reliabilityGrade: 'B', reliabilityNote: 'Scraped from TrendForce pages; layout changes can interrupt parsing.',
    ragScope: 'Memory cost — NAND flash, NAND wafer, and memory-card spot prices.',
    fallback: 'Daily history file; last session persists.',
    endpointUrl: 'https://www.trendforce.com/price/flash/flash_spot',
  },
  tftLcd: {
    name: 'TFT-LCD Panel Prices', provider: 'TrendForce',
    ourCadenceMs: 24 * 3600e3, sourceCadence: 'Monthly / half-monthly', criticalLagThresholdMs: 72 * 3600e3,
    upstreamLagMs: 14 * 24 * 3600e3, upstreamLagText: '~half-month to monthly',
    upstreamLagNote: 'TrendForce publishes public TFT-LCD panel price tables on a monthly or half-monthly cycle. We snapshot each run and forward-fill the panel price until the next posted update.',
    reliabilityGrade: 'B', reliabilityNote: 'Scraped from TrendForce pages; layout changes can interrupt parsing.',
    ragScope: 'Display component costs — large-size and smartphone TFT-LCD panel prices.',
    fallback: 'Daily history file; last posted panel price persists.',
    endpointUrl: 'https://www.trendforce.com/price/lcd/panel',
  },
  aws: {
