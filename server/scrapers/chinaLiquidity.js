...
async function updateChinaLiquidity(days = 730) {
  ...
  const settled = await Promise.allSettled([fetchTurnover(start, end), fetchM2Yoy()]);
  const errors = {};
...
module.exports = { updateChinaLiquidity, readChinaLiquidity, _test: { parseTurnoverKlines, deriveM2Yoy, assemble } };
